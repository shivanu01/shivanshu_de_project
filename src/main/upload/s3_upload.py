...
class UploadToS3:
    ...
    def upload_to_s3(self,s3_directory,s3_bucket,local_file_path):
        ...
        try:
            for root,dir,files in os.walk(local_file_path):
                for file in files:
                    file_full_local_path = os.path.join(root,file)
                    s3_key = f"{s3_prefix}/{file}"
                    self.s3_client.upload_file(file_full_local_path,s3_bucket,s3_key)
